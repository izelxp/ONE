...
public class Mano {
    ...
    /**
    * SE MODIFICA EL METODO PARA RECIBIR LA CARTA QUE ESTA EN LA MESA ACTUALMENTE
    * Y PODER HACER TODAS LAS COMPARACIONES CORRESPONDIENTES, Y NO SOLO POR COLOR O NUMERO
    * NOS APOYAREMOS DE UNA NUEVO PARAMETRO "cartaMesa" 
    * @param cartaMesa CARTA QUE ESTA ACTUALMENTE EN LA MESA
    *  ULTIMA MODIFICACIÓN 8-MARZO-26 BY JAV 
    ...
    public List<Integer> obtenerCartasValidas(Carta cartaMesa) {
        ...
        for (int i = 0; i < cartas.size(); i++) {
            if (cartas.get(i).esValida(cartaMesa)) {
                validas.add(i);
            }
        }
        return validas;
